...
#if defined(_PLATFORM_IS_LINUX_) || MBED_CONF_RTOS_PRESENT
...
#define PRODUCT_KEY             "a1SaLQJIZzr"
#define DEVICE_NAME             "tRigXE0ATjmiyJlv6aPp"
#define DEVICE_SECRET           "O2sWqW5FuiVt062GAp831clPkXnBx8H3"
...
#elif MBED_CONF_RTOS_PRESENT
    ret = HAL_ThreadCreate(&pid1, thread_subscribe1, pclient, NULL, NULL);
#endif
    if (ret != 0) {
...
#elif MBED_CONF_RTOS_PRESENT
    ret = HAL_ThreadCreate(&pid2, thread_subscribe2, pclient, NULL, NULL);
#endif
    if (ret != 0) {
        printf("pthread_create failed!\n");
        return;
...
#elif MBED_CONF_RTOS_PRESENT
        printf("thread<ID not support>:ret = %d\n", ret);
#endif
        HAL_SleepMs(300);
    }
...
#elif MBED_CONF_RTOS_PRESENT
    ret = HAL_ThreadCreate(&pid1, thread_publish1, pclient, NULL, NULL);
#endif
    if (ret != 0) {
        printf("pthread_create failed!\n");
        return;
...
#elif MBED_CONF_RTOS_PRESENT
    ret = HAL_ThreadCreate(&pid2, thread_publish2, pclient, NULL, NULL);
#endif
    if (ret != 0) {
        printf("pthread_create failed!\n");
        return;
...
int mqtt_client(void)
{
...
#elif MBED_CONF_RTOS_PRESENT
    HAL_ThreadCreate(&pid1, thread_yield, pclient, NULL, NULL);
#endif
    ...
    /* Take into consideration this sleep time and lifetime of thread_subscribe1/2 and thread_publish1/2
     * If we wake up before end of thread_subscribe1/2 and thread_publish1/2, thread_subscribe1/2 and
     * thread_publish1/2 would run into hard fault because main thread exits before these threads and
     * resources for them are being recycled. */
#if defined(_PLATFORM_IS_LINUX_)
    HAL_SleepMs(100000);
#elif MBED_CONF_RTOS_PRESENT
    HAL_SleepMs(200000);
